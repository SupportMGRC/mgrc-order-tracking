...
@section('content')
    ...
    <div class="card mb-4 {{ $order->time_sensitive ? 'ribbon-box border border-danger' : '' }}">
        ...
        ($order->status == 'ready' || $order->status == 'delivered' ? 'completed' : '') 
                            ...
                            title="Order is being prepared with batch information">
                            <div class="order-track-icon">
        ...
        ($order->status == 'delivered' ? 'completed' : '') 
                            ...
                            title="Order is ready for pickup or delivery">
                            <div class="order-track-icon">
                        ...
                        <div class="order-track-step {{ $order->status == 'delivered' ? 'completed' : '' }}"
                            data-bs-toggle="tooltip" data-bs-placement="top"
                            title="Order has been delivered to the customer">
                            <div class="order-track-icon">
                            ...
                            <div class="mt-2 text-center">
                                <div
                                    class="text-{{ $order->delivery_type === 'delivery' ? 'success' : 'primary' }} fw-medium">
                                    <i
                                        class="ri-{{ $order->delivery_type === 'delivery' ? 'truck-line' : 'user-location-line' }} align-bottom me-1"></i>
                        ...
                        <div class="order-track-step {{ $order->status == 'cancel' ? 'active' : '' }}"
                            data-bs-toggle="tooltip" data-bs-placement="top" title="Order has been canceled">
                            <div class="order-track-icon">
            ...
            @if($order->status != 'delivered' && $order->status != 'cancel')
                <div class="text-center">
                    ...
                    @elseif($order->status === 'ready')
                        @if(Auth::user()->department === 'Admin & Human Resource' || Auth::user()->department === 'Dispatcher' || Auth::user()->role === 'admin' || Auth::user()->role === 'superadmin')
            ...
            <div class="card-body">
                @if($order->hasPhotos())
                    <div class="mb-3">
                        ...
                        @if(count($allPhotos) === 1)
                            <img src="{{ asset('storage/order_photos/' . $allPhotos[0]) }}" alt="Order Photo"
                                class="img-fluid rounded shadow-sm" style="max-width: 300px; cursor:pointer;" data-bs-toggle="modal"
                                data-bs-target="#viewOrderPhotoModal" data-photo="{{ $allPhotos[0] }}">
                        @else
                            <div class="row">
                                @foreach($allPhotos as $index => $photo)
                                    <div class="col-md-4 mb-3">
                                        <div class="position-relative">
                                            <img src="{{ asset('storage/order_photos/' . $photo) }}" alt="Order Photo {{ $index + 1 }}"
                                                class="img-fluid rounded shadow-sm" style="max-width: 100%; cursor:pointer;"
                                                data-bs-toggle="modal" data-bs-target="#viewOrderPhotoModal" data-photo="{{ $photo }}">
                                            @if($order->status !== 'delivered')
                                                <div class="position-absolute top-0 end-0 p-1">
                                                    <button type="button" class="btn btn-sm btn-danger rounded-circle" data-bs-toggle="modal"
                                                        data-bs-target="#deleteSpecificPhotoModal{{ $index }}" title="Delete this photo">
                    ...
                    @if($order->status === 'delivered')
                        ...
                        <div class="alert alert-info mb-3">
                            <i class="ri-information-line me-2"></i> <strong>Delivered Order:</strong> Photos cannot be deleted for
                            record-keeping purposes. You can still add more photos if needed.
                        </div>
                        <button type="button" class="btn btn-outline-primary btn-sm mb-2" data-bs-toggle="collapse"
                            data-bs-target="#addPhotoFormDelivered">
                        ...
                        <div class="collapse" id="addPhotoFormDelivered">
                            <form action="{{ route('orders.upload.photo', $order->id) }}" method="POST" enctype="multipart/form-data"
                                id="photoUploadFormDelivered">
                                @csrf
                                <div class="mb-3">
                                    <label for="order_photo_delivered" class="form-label">Add More Photos</label>
                                    <div class="position-relative">
                                        <input type="file" class="form-control" id="order_photo_delivered" name="order_photos[]"
                                            accept="image/jpeg,image/jpg,image/png,image/gif,image/webp,image/heic,image/heif" multiple
                                            required>
                                        <div id="file-size-info-delivered" class="form-text text-muted">
                                            <small>Supports: JPEG, PNG, GIF, WebP, HEIC. Max size: 10MB each. Images will be
                                    ...
                                    <div id="image-preview-delivered" class="mt-3" style="display: none;">
                                        <img id="preview-img-delivered" src="" alt="Preview" class="img-fluid rounded shadow-sm"
                    ...
                    @elseif($order->status !== 'delivered')
                        <div class="d-flex gap-2 mb-2">
                            ...
                            <button type="button" class="btn btn-outline-primary btn-sm" data-bs-toggle="collapse"
                                data-bs-target="#editPhotoForm">{{ $order->status === 'preparing' ? 'Add More' : 'Edit' }}</button>
                            <!-- Delete button shows modal -->
                            <button type="button" class="btn btn-outline-danger btn-sm" data-bs-toggle="modal"
                                data-bs-target="#deletePhotoModal">Delete</button>
                        </div>
                        <div class="collapse" id="editPhotoForm">
                            <form action="{{ route('orders.upload.photo', $order->id) }}" method="POST" enctype="multipart/form-data"
                                id="editPhotoUploadForm">
                                @csrf
                                <div class="mb-3">
                                    <label for="order_photo_edit"
                                        class="form-label">{{ $order->status === 'preparing' ? 'Add More Photos' : 'Replace Photo' }}
                                        <span class="text-danger">*</span></label>
                                    <div class="position-relative">
                                        <input type="file" class="form-control" id="order_photo_edit"
                                            name="{{ $order->status === 'preparing' ? 'order_photos[]' : 'order_photo' }}"
                                            accept="image/jpeg,image/jpg,image/png,image/gif,image/webp,image/heic,image/heif" {{ $order->status === 'preparing' ? 'multiple' : '' }} required>
                                        <div class="form-text text-muted">
                                            <small>Supports: JPEG, PNG, GIF, WebP, HEIC. Max size:
                                                10MB{{ $order->status === 'preparing' ? ' each. Images will be automatically compressed.' : '. Image will be automatically compressed.' }}</small>
                                        </div>
                                    </div>

                                    <!-- Image Preview for Edit -->
                                    <div id="edit-image-preview" class="mt-3" style="display: none;">
                                        <img id="edit-preview-img" src="" alt="Preview" class="img-fluid rounded shadow-sm"
                                            style="max-width: 200px; max-height: 200px;">
                                        <div class="mt-2">
                ...
                @else
                    @if($order->status === 'delivered')
                        ...
                        <div class="alert alert-info mb-3">
                            <i class="ri-information-line me-2"></i> <strong>Delivered Order:</strong> You can upload photos for
                            record-keeping. Once uploaded, photos cannot be deleted.
                        </div>
                        <form action="{{ route('orders.upload.photo', $order->id) }}" method="POST" enctype="multipart/form-data"
                            id="photoUploadFormDeliveredNew">
                            @csrf
                            <div class="mb-3">
                                <label for="order_photo_delivered_new" class="form-label">Upload Photo(s) <span
                                        class="text-danger">*</span></label>
                                <div class="position-relative">
                                    <input type="file" class="form-control" id="order_photo_delivered_new" name="order_photos[]"
                                        accept="image/jpeg,image/jpg,image/png,image/gif,image/webp,image/heic,image/heif" multiple
                                        required>
                                    <div id="file-size-info-delivered-new" class="form-text text-muted">
                                        <small>Supports: JPEG, PNG, GIF, WebP, HEIC. Max size: 10MB each. Images will be automatically
                                            compressed.</small>
                                    </div>
                                </div>
                                ...
                                <div id="image-preview-delivered-new" class="mt-3" style="display: none;">
                                    <img id="preview-img-delivered-new" src="" alt="Preview" class="img-fluid rounded shadow-sm"
                    ...
                    @elseif($order->status === 'preparing')
                        <div class="alert alert-info mb-3">
                            <i class="ri-information-line me-2"></i> You can upload photos while preparing the order. Upload photos to
                            track the preparation progress.
                        </div>
                        <form action="{{ route('orders.upload.photo', $order->id) }}" method="POST" enctype="multipart/form-data"
                            id="photoUploadFormPreparing">
                            @csrf
                            <div class="mb-3">
                                <label for="order_photo_preparing" class="form-label">Upload Photo(s) <span
                                        class="text-danger">*</span></label>
                                <div class="position-relative">
                                    <input type="file" class="form-control" id="order_photo_preparing" name="order_photos[]"
                                        accept="image/jpeg,image/jpg,image/png,image/gif,image/webp,image/heic,image/heif" multiple
                                        required>
                                    <div id="file-size-info-preparing" class="form-text text-muted">
                                        <small>Supports: JPEG, PNG, GIF, WebP, HEIC. Max size: 10MB each. Images will be automatically
                                            compressed to save space.</small>
                                    </div>
                                </div>

                                <!-- Image Preview -->
                                <div id="image-preview-preparing" class="mt-3" style="display: none;">
                                    <img id="preview-img-preparing" src="" alt="Preview" class="img-fluid rounded shadow-sm"
                                        style="max-width: 300px; max-height: 300px;">
                                    <div class="mt-2">
                                        <small class="text-muted">Preview - Image will be uploaded when you click submit</small>
                                    </div>
                    ...
                    @else
                        ...
                        <form action="{{ route('orders.upload.photo', $order->id) }}" method="POST" enctype="multipart/form-data"
                            ...
                            <div class="mb-3">
                                ...
                                <div class="position-relative">
                                    <input type="file" class="form-control" id="order_photo" name="order_photos[]"
                                        accept="image/jpeg,image/jpg,image/png,image/gif,image/webp,image/heic,image/heif" multiple
                                        required>
                                ...
                                <div id="image-preview" class="mt-3" style="display: none;">
                                    <img id="preview-img" src="" alt="Preview" class="img-fluid rounded shadow-sm"
                                        style="max-width: 300px; max-height: 300px;">
                                    <div class="mt-2">
                                        <small class="text-muted">Preview - Image will be uploaded when you click submit</small>
                                    </div>
    ...
    <!-- Dispatcher Evidence Section -->
    @if($order->status === 'delivered')
        <div class="card mb-4">
            <div class="card-header d-flex align-items-center">
                <h5 class="card-title mb-0 flex-grow-1"><i class="ri-truck-line align-middle me-1 text-muted"></i> Dispatcher
                    Evidence</h5>
            </div>
            <div class="card-body">
                @if($order->delivery_photos && count($order->delivery_photos) > 0)
                    <div class="row">
                        @foreach($order->delivery_photos as $index => $photo)
                            <div class="col-md-4 mb-3">
                                <div class="position-relative">
                                    <img src="{{ asset('storage/order_photos/' . $photo) }}" alt="Delivery Photo {{ $index + 1 }}"
                                        class="img-fluid rounded shadow-sm" style="max-width: 100%; cursor:pointer;"
                                        data-bs-toggle="modal" data-bs-target="#viewOrderPhotoModal" data-photo="{{ $photo }}">
                                    @if(Auth::user()->department === 'Dispatcher' || Auth::user()->role === 'admin' || Auth::user()->role === 'superadmin')
                                        <div class="position-absolute top-0 end-0 p-1">
                                            <button type="button" class="btn btn-sm btn-danger rounded-circle" data-bs-toggle="modal"
                                                data-bs-target="#deleteDeliveryPhotoModal{{ $index }}" title="Delete this photo">
                                                <i class="ri-close-line"></i>
                                            </button>
                                        </div>
                                    @endif
                ...
                @else
                    <div class="alert alert-info mb-3">
                        <i class="ri-information-line me-2"></i> No delivery evidence photos uploaded yet.
                    </div>
                @endif
                ...
                @if(Auth::user()->department === 'Dispatcher' || Auth::user()->role === 'admin' || Auth::user()->role === 'superadmin')
                    <div class="mt-2">
                        <button type="button" class="btn btn-outline-primary btn-sm mb-2" data-bs-toggle="collapse"
                            data-bs-target="#addDeliveryPhotoForm">
                            <i class="ri-add-line me-1"></i> Add Delivery Photo(s)
                        </button>
                        <div class="collapse" id="addDeliveryPhotoForm">
                            <form action="{{ route('orders.upload.delivery-photo', $order->id) }}" method="POST"
                                enctype="multipart/form-data" id="deliveryPhotoUploadForm">
                                @csrf
                                <div class="mb-3">
                                    <label for="delivery_photos" class="form-label">Upload Delivery Evidence <span
                                            class="text-danger">*</span></label>
                                    <input type="file" class="form-control" id="delivery_photos" name="delivery_photos[]"
                                        accept="image/*" multiple required>
                                    <div class="form-text text-muted">
                                        <small>Supports: JPEG, PNG, GIF, WebI, HEIC. Max size: 10MB each.</small>
                                    </div>
                                </div>
                                <button type="submit" class="btn btn-primary btn-sm">
                                    <i class="ri-upload-cloud-2-line align-middle me-1"></i> Upload Evidence
                                </button>
                            </form>
                        </div>
                    </div>
                ...
                <form action="{{ route('orders.delete.photo', $order->id) }}" method="POST">
                    ...
                    <div class="modal-body">
                        <p>Are you sure you want to delete
                            {{ $order->hasPhotos() && count($order->getAllPhotos()) === 1 ? 'this photo' : 'all photos' }}?
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="submit" class="btn btn-danger">Delete
                            {{ $order->hasPhotos() && count($order->getAllPhotos()) === 1 ? 'Photo' : 'All Photos' }}</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    @if($order->hasPhotos() && count($order->getAllPhotos()) > 1)
        @foreach($order->getAllPhotos() as $index => $photo)
            <div class="modal fade" id="deleteSpecificPhotoModal{{ $index }}" tabindex="-1"
                aria-labelledby="deleteSpecificPhotoModalLabel{{ $index }}" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        <div class="modal-header bg-soft-danger">
                            <h5 class="modal-title" id="deleteSpecificPhotoModalLabel{{ $index }}">Delete Photo {{ $index + 1 }}
                            </h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <form action="{{ route('orders.delete.specific.photo', [$order->id, $photo]) }}" method="POST">
                            ...
                            <div class="modal-body">
                                <div class="text-center mb-3">
                                    <img src="{{ asset('storage/order_photos/' . $photo) }}" alt="Photo to delete"
    ...
    <!-- Delivery Photo Specific Delete Modals -->
    @if($order->delivery_photos && count($order->delivery_photos) > 0)
        @foreach($order->delivery_photos as $index => $photo)
            <div class="modal fade" id="deleteDeliveryPhotoModal{{ $index }}" tabindex="-1" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        <div class="modal-header bg-soft-danger">
                            <h5 class="modal-title">Delete Delivery Photo {{ $index + 1 }}</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <form action="{{ route('orders.delete.delivery-photo', [$order->id, $photo]) }}" method="POST">
                            @csrf
                            @method('DELETE')
                            <div class="modal-body">
                                <div class="text-center mb-3">
                                    <img src="{{ asset('storage/order_photos/' . $photo) }}" alt="Delivery photo to delete"
                                        class="img-fluid rounded" style="max-width: 200px;">
                                </div>
                                <p class="text-center">Are you sure you want to delete this delivery photo? This action cannot be
                                    undone.</p>
                            </div>
                            <div class="modal-footer">
                                <button type="button" class="btn btn-light" data-bs-dismiss="modal">Cancel</button>
    ...
    <div class="modal fade" id="viewOrderPhotoModal" tabindex="-1" aria-labelledby="viewOrderPhotoModalLabel"
        aria-hidden="true">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body text-center p-3">
                    <img id="modalPhotoImg" src="" alt="Order Photo" class="img-fluid rounded shadow"
                        style="max-height: 75vh; width: auto; cursor: zoom-in;">
                    <div class="mt-2">
    ...
    <div class="row">
                ...
                <div class="card-header">
                    <div class="d-flex align-items-center flex-wrap">
                        ...
                        <div class="flex-shrink-0 mt-2 mt-md-0">
                            ...
                            <button type="button" class="btn btn-info btn-sm me-2 mb-2" data-bs-toggle="modal"
                                data-bs-target="#updateDeliveryDateTimeModal">
                                            ...
                                            <td>
                                                @if($product->pivot->status === 'ready')
                                                    <button type="button" class="btn btn-sm btn-soft-success" data-bs-toggle="modal"
                                                        data-bs-target="#markProductNotReadyModal{{ $product->pivot->id }}">
                                                ...
                                                @else
                                                    <button type="button" class="btn btn-sm btn-soft-danger" data-bs-toggle="modal"
                                                        data-bs-target="#markProductReadyModal{{ $product->pivot->id }}">
                        ...
                        <div class="text-center mt-4">
                            ...
                            @elseif(!$allProductsReady || count($order->products) == 0)
                                <div class="alert alert-info">
                                    <i class="ri-information-line me-2"></i> Mark all products as ready to proceed with order status
                                    change.
                                </div>
                            @endif
                        </div>
        ...
        <div class="col-xl-3 col-lg-4 col-md-12">
            <div class="card">
                <div class="card-header">
                    <div class="d-flex flex-wrap">
                        ...
                        <div class="flex-shrink-0 mt-2 mt-md-0">
                            <a href="{{ route('customers.show', $order->customer_id) }}" class="link-secondary">View
                                Profile</a>
                        </div>
                    </div>
                </div>
                <div class="card-body">
                    <ul class="list-unstyled mb-0 vstack gap-3">
                            ...
                            <div class="d-flex align-items-center">
                                <div class="flex-shrink-0">
                                    @if($order->customer->profile_image)
                                        <img src="{{ asset('assets/images/users/' . $order->customer->profile_image) }}" alt=""
                        ...
                        <li><i
                                class="ri-mail-line me-2 align-middle text-muted fs-16"></i>{{ $order->customer->email ?? 'N/A' }}
                        </li>
                        <li><i
                                class="ri-phone-line me-2 align-middle text-muted fs-16"></i>{{ $order->customer->phoneNo ?? 'N/A' }}
                        </li>

                        @if($order->order_placed_by)
                            <li><i class="ri-user-received-2-line me-2 align-middle text-muted fs-16"></i>Placed by:
                                {{ $order->order_placed_by }}
                            </li>
                        @endif
                        @if($order->delivered_by)
                            <li><i class="ri-truck-line me-2 align-middle text-muted fs-16"></i>Delivered by:
                                {{ $order->delivered_by }}
                            </li>
                        @endif
                        @if($order->temp_before_delivery)
                            <li><i class="ri-temp-cold-line me-2 align-middle text-muted fs-16"></i>Temp Before Delivery:
                                {{ $order->temp_before_delivery }}
                            </li>
                        @endif
                        @if($order->temp_after_delivery)
                            <li><i class="ri-temp-hot-line me-2 align-middle text-muted fs-16"></i>Temp After Delivery:
                                {{ $order->temp_after_delivery }}
                            </li>
                        @endif
                    </ul>
                </div>
            ...
            <div class="card">
                <div class="card-header">
                    <h5 class="card-title mb-0"><i class="ri-map-pin-user-line align-middle me-1 text-muted"></i> Delivery
                        Address</h5>
                </div>
                <div class="card-body">
    ...
    <div class="modal fade" id="markPreparingModal" tabindex="-1" aria-labelledby="markPreparingModalLabel"
        aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <form action="{{ route('orders.update.status', $order->id) }}" method="POST">
                    ...
                    <div class="modal-body">
                        <div class="alert alert-info mb-3">
                            <i class="ri-information-line me-2"></i> Marking as "Preparing" means batch information has been
                            assigned and order preparation is in progress.
                        </div>
    ...
    <div class="modal fade" id="markOrderReadyModal" tabindex="-1" aria-labelledby="markOrderReadyModalLabel"
        aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <form action="{{ route('orders.mark.ready', $order->id) }}" method="POST">
                    ...
                    <div class="modal-body">
                        <div class="alert alert-info mb-3">
                            <i class="ri-information-line me-2"></i> Marking as "Ready" means the order has been prepared
                            and is ready for delivery or pickup.
                        </div>
        ...
        <div class="modal fade" id="markProductReadyModal{{ $product->pivot->id }}" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <form action="{{ route('orders.product.ready', ['order' => $order->id, 'product' => $product->id]) }}"
                        method="POST" class="product-ready-form">
                        @csrf
                        @method('PATCH')
                        ...
                        <div class="modal-body">
                            ...
                            <p class="text-center">Are you sure you want to mark <strong>{{ $product->name }}</strong> as ready?
                            </p>
                ...
                <div class="modal-content">
                    ...
                    <form action="{{ route('orders.product.ready', ['order' => $order->id, 'product' => $product->id]) }}"
                        ...
                        <div class="modal-body">
                            ...
                            <p class="text-center">Are you sure you want to mark <strong>{{ $product->name }}</strong> as not
                                ready?</p>
    ...
    <div class="modal fade" id="markDeliveredModal" tabindex="-1" aria-labelledby="markDeliveredModalLabel"
        aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                ...
                <form action="{{ route('orders.update.status', $order->id) }}" method="POST" enctype="multipart/form-data"
                    id="markDeliveredForm">
                    @csrf
                    @method('PATCH')
                    <div class="modal-body">
                        <div class="alert alert-info mb-3">
                            <i class="ri-information-line me-2"></i> Marking as "Delivered" means the order has been
                            successfully delivered to the customer.
                        </div>

                        <!-- Delivery Photo Upload -->
                        <div class="mb-3">
                            <label for="delivery_photo" class="form-label">Delivery Photo <span
                                    class="text-danger">*</span></label>
                            <input type="file" class="form-control" id="delivery_photo" name="delivery_photo"
                                accept="image/jpeg,image/jpg,image/png,image/gif,image/webp,image/heic,image/heif" required>
                            <small class="text-muted">Take a photo of the delivered product. Supports: JPEG, PNG, GIF, WebP,
                                HEIC. Max size: 10MB.</small>

                            <!-- Image Preview -->
                            <div id="delivery-image-preview" class="mt-3" style="display: none;">
                                <img id="delivery-preview-img" src="" alt="Preview" class="img-fluid rounded shadow-sm"
                                    style="max-width: 200px; max-height: 200px;">
                                <div class="mt-2">
                                    <small class="text-muted">Preview - Image will be uploaded when you confirm
                                        delivery</small>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    <label for="temp_before_delivery" class="form-label">Temperature Before Delivery <span
                                            class="text-danger">*</span></label>
                                    <div class="input-group">
                                        <input type="text" class="form-control" id="temp_before_delivery"
                                            name="temp_before_delivery" placeholder="e.g., -20°C" required>
                                        <span class="input-group-text"><i class="ri-temp-cold-line"></i></span>
                                    </div>
                                    <small class="text-muted">Temperature of product before dispatch</small>
                                </div>
                            </div>
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    <label for="temp_after_delivery" class="form-label">Temperature After Delivery <span
                                            class="text-danger">*</span></label>
                                    <div class="input-group">
                                        <input type="text" class="form-control" id="temp_after_delivery"
                                            name="temp_after_delivery" placeholder="e.g., -18°C" required>
                                        <span class="input-group-text"><i class="ri-temp-hot-line"></i></span>
                                    </div>
                                    <small class="text-muted">Temperature of product upon delivery</small>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <small class="text-muted">This records who delivered or received the order, not who placed
                                it.</small>
                        </div>
                        <div class="mb-3">
                            ...
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="delivery_type"
                                    id="delivery_type_delivery" value="delivery" {{ $order->delivery_type === 'delivery' ? 'checked' : '' }} required>
                            ...
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="delivery_type" id="delivery_type_self"
                                    value="self_collect" {{ $order->delivery_type === 'self_collect' ? 'checked' : '' }}
                                    required>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control flatpickr-input" id="delivery_datetime"
                                name="delivery_datetime" data-provider="flatpickr" data-date-format="d.m.Y h:i K"
    ...
    <div class="modal fade" id="updateDeliveryDateTimeModal" tabindex="-1"
        aria-labelledby="updateDeliveryDateTimeModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <form action="{{ route('orders.delivery.datetime.update', $order->id) }}" method="POST"
                    id="deliveryDateTimeForm">
                    @csrf
                    @method('PATCH')
                    <div class="modal-body">
                        <div class="alert alert-info mb-3">
                            <i class="ri-information-line me-2"></i> Update the delivery date, reach client time, ready
                            time, and collection date for this order.
                        </div>
                        <div class="row">
                            <div class="col-lg-12">
                                <div class="mb-3">
                                    <label for="pickup_delivery_date"
                                        class="form-label">{{ $order->delivery_type === 'delivery' ? 'Delivery' : 'Self Collect' }}
                                        Date <span class="text-danger">*</span></label>
                                    <input type="text"
                                        class="form-control @error('pickup_delivery_date') is-invalid @enderror"
                                        id="pickup_delivery_date" name="pickup_delivery_date" data-provider="flatpickr"
                                        data-date-format="Y-m-d" data-mindate="today"
                                        value="{{ $order->pickup_delivery_date ? $order->pickup_delivery_date->format('Y-m-d') : '' }}"
                                        required>
                            ...
                            <div class="col-lg-6">
                                <div class="mb-3">
                                    <label for="collection_date_update" class="form-label">Collection Date <span
                                            class="text-muted">(Optional)</span></label>
                                    <input type="text" class="form-control @error('collection_date') is-invalid @enderror"
                                        id="collection_date_update" name="collection_date" data-provider="flatpickr"
                            ...
                            <div class="col-lg-6">
                                <div class="mb-3">
                                    <label for="item_ready_time_display" class="form-label">Ready Time <span
                                            class="text-danger">*</span></label>
                                    <input type="text" class="form-control @error('item_ready_time') is-invalid @enderror"
                                        id="item_ready_time_display" data-provider="flatpickr" data-enable-time="true" data-no-calendar="true" data-date-format="h:i K"
                                        placeholder="Select ready time"
                                        value="{{ $order->item_ready_at ? \Carbon\Carbon::parse($order->item_ready_at)->format('h:i A') : '' }}"
                                        required>
                            ...
                            <div class="col-lg-6">
                                <div class="mb-3">
                                    <label for="pickup_delivery_time_display"
                                        class="form-label">{{ $order->delivery_type === 'delivery' ? 'Reach Client' : 'Self Collect' }}
                                        Time <span class="text-danger">*</span></label>
                                    <input type="text"
                                        class="form-control @error('pickup_delivery_time') is-invalid @enderror"
                                        id="pickup_delivery_time_display" data-provider="flatpickr" data-enable-time="true" data-no-calendar="true" data-date-format="h:i K"
                                        placeholder="Select time"
                                        value="{{ \Carbon\Carbon::parse($order->pickup_delivery_time)->format('h:i A') }}"
                                        required>
    ...
    d    ocument.addEventListener('DOMContentLoaded', function () {
                ...
                function setupPhotoUpload(fileInputId, previewId, progressId, formId) {
                    ...
                    // Delivery photo preview for Mark as Delivered modal
                    const deliveryPhotoInput = document.getElementById('delivery_photo');
                    const deliveryPreviewContainer = document.getElementById('delivery-image-preview');
                    const deliveryPreviewImg = document.getElementById('delivery-preview-img');

                    if (deliveryPhotoInput && deliveryPreviewContainer && deliveryPreviewImg) {
                        deliveryPhotoInput.addEventListener('change', function(e) {
                            if (e.target.files && e.target.files[0]) {
                                const reader = new FileReader();
                                reader.onload = function(event) {
                                    deliveryPreviewImg.src = event.target.result;
                                    deliveryPreviewContainer.style.display = 'block';
                                };
                                reader.readAsDataURL(e.target.files[0]);
                            } else {
                                deliveryPreviewContainer.style.display = 'none';
                            }
                        });
                    }
